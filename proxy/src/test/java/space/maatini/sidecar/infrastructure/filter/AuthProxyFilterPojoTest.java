...
class AuthProxyFilterPojoTest {
    ...
    void testFilter_Error() {
        ...
        when(processor.process(any(SidecarRequest.class)))
                .thenReturn(Uni.createFrom().item(ProcessingResult.error("Boom")));
    ...
    void testFilter_Forbidden() {
        ...
        when(context.getUriInfo().getPath()).thenReturn("/forbidden");
        when(context.getMethod()).thenReturn("GET");
        when(context.getHeaders()).thenReturn(new MultivaluedHashMap<>());
        when(context.getUriInfo().getQueryParameters()).thenReturn(new MultivaluedHashMap<>());

        space.maatini.sidecar.domain.model.PolicyDecision decision = space.maatini.sidecar.domain.model.PolicyDecision
                .deny("Denied");
        when(processor.process(any())).thenReturn(Uni.createFrom().item(ProcessingResult.forbidden(decision)));
    ...
    @Test
    void testFilter_Proceed_SetsAuthContext() {
        ContainerRequestContext context = mock(ContainerRequestContext.class);
        when(context.getUriInfo()).thenReturn(mock(UriInfo.class));
        when(context.getUriInfo().getPath()).thenReturn("/proceed");
        when(context.getMethod()).thenReturn("GET");
        when(context.getHeaders()).thenReturn(new MultivaluedHashMap<>());
        when(context.getUriInfo().getQueryParameters()).thenReturn(new MultivaluedHashMap<>());

        space.maatini.sidecar.domain.model.AuthContext auth = space.maatini.sidecar.domain.model.AuthContext.builder()
                .userId("user1").build();
        when(processor.process(any())).thenReturn(Uni.createFrom().item(ProcessingResult.proceed(auth)));

        Response res = filter.filter(context).await().indefinitely();
        assertNull(res);
        verify(context).setProperty("auth.context", auth);
    }
}
